...
export async function scrapeTikTokSlideshow(url: string): Promise<TikTokScrapeResult> {
  ...
  try {
    ...
    // Debug: Log the entire post structure to see what fields are available
    console.log('TikTok post data:', JSON.stringify(post, null, 2));
    ...
    if (post.isSlideshow && post.slideshowImageLinks && Array.isArray(post.slideshowImageLinks)) {
      slideImages = post.slideshowImageLinks.map((img: { tiktokLink?: string; downloadLink?: string }) => 
    ...
    // Fallback: Check other possible field names
      ...
      slideImages = post.imagePost.images.map((img: { imageURL?: { urlList?: string[] } }) => 
      ...
      slideImages = post.images.map((img: string | { url?: string }) => 
    ...
    if (slideImages.length === 0) {
      if (post.videoMeta?.duration > 0 || post.videoUrl || post.video?.playAddr) {
        throw new Error('This TikTok post is a video, not a Photo Mode slideshow. Video-only posts are not supported in this MVP.');
      }
      throw new Error('Could not extract slideshow images from this TikTok post. The post may not be a Photo Mode slideshow.');
    }
